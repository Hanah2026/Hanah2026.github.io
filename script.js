  ...
  panel.innerHTML = `
    ...
    <div class="welcome-card" id="recordPhotoCard">
      ...
      <div class="photo-upload-row">
        ...
        <div><input id="studentPhotoFile" type="file" accept="image/jpeg,image/png,image/webp" onchange="if (this.files && this.files.length && window.currentRecordStudentId && typeof window.uploadStudentPhoto === 'function') { window.uploadStudentPhoto(window.currentRecordStudentId); }">
        <button id="uploadStudentPhoto" class="btn" type="button" onclick="document.getElementById('studentPhotoFile').click(); return false;">UPLOAD PHOTO</button>
...
// Robust student photo upload fallback: works even if the dynamic panel event binding is missed.
(function(){
  document.addEventListener("click", function(e){
    const btn = e.target && e.target.closest ? e.target.closest("#uploadStudentPhoto") : null;
    if (!btn) return;
    e.preventDefault();
    const input = document.getElementById("studentPhotoFile");
    if (input) input.click();
  }, true);
  document.addEventListener("change", function(e){
    if (!e.target || e.target.id !== "studentPhotoFile") return;
    if (e.target.files && e.target.files.length && window.currentRecordStudentId && typeof window.uploadStudentPhoto === "function") {
      window.uploadStudentPhoto(window.currentRecordStudentId);
    }
  }, true);
})();
